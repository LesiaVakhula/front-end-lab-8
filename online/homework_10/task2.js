...
function fighter(object) {
    let defObject = object,
        combatResul = {
            wins: 0,
            lose: 0
        };

    function fight(objectEnemy) {
        if (objectEnemy.block()) {
            console.log(`Fighter ${objectEnemy.getStats().name} was able to protect`);
            return false
        } else {
            objectEnemy.getStats().hp = (objectEnemy.getStats().hp - defObject.attack);
            console.log(`Fighter ${objectEnemy.getStats().name} wasn't able to protect`);
            if (objectEnemy.getStats().hp === 0) {
                combatResul.wins++;
                objectEnemy.getCombatHistory().lose++;
            }
            return true
        }
    }
    ...
    return{
        getName: ()=>defObject.name,
        fight: fight,
        block: ()=>(Math.round(Math.random()*10)) <= 5,
        getStats: ()=>defObject,
        getCombatHistory: ()=>combatResul
    }
}
